...
struct SplashSView: View {
    ...
    var body: some View {
        ...
        VStack {
            // Combine multiple `Text` views with different colors to create the multi-colored effect in one line.
            (
              Text("One ").foregroundColor(.mint) + // Set the color for "One"
                Text("Picture, ").foregroundColor(.green) + // Set the color for "Picture,"
              Text("Infinite ").foregroundColor(.brown) + // Set the color for "Infinite"
                Text("Words!").foregroundColor(.purple) // Set the color for "Words!"
            )
            ...
            .animation(.easeInOut(duration: 1.0), value: animate) // Apply a smooth animation with ease in and out transition over 7 seconds.
        }
        .onAppear {
